...
if (isset($_SESSION['loggedin']))
{
	...
	if($pieza == '--')
	{
		$con = $conexion->query("UPDATE parte SET nom_tec = '$nombre_tecnico', fecha_resolucion = CURRENT_DATE(), hora_resolucion = CURRENT_TIME(), not_tec = '$not_ant', resuelto = 1, tec_res = $id_emp  
		WHERE id_part = $id_part and (tec_res='$id_emp' or tec_res is null) and resuelto=0");
	}
	else
	{
		$con = $conexion->query("UPDATE parte SET pieza= '$pieza', nom_tec = '$nombre_tecnico', fecha_resolucion = CURRENT_DATE(), hora_resolucion = CURRENT_TIME(), not_tec = '$not_ant', resuelto = 1, tec_res = $id_emp  
		WHERE id_part = $id_part and (tec_res='$id_emp' or tec_res is null) and resuelto=0");
	}
